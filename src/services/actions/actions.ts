...
export const onReset = createAsyncThunk<
  ...
  TUser,
  { rejectValue: TError }
>("user/onReset", async function (user, { rejectWithValue }) {
  const response = await resetRequest(user);
  if (!response.ok) {
    return rejectWithValue({
      status: response.status,
      message: "Server Error, take a look on method onReset",
    });
  }
  const data: TResponseWithoutPayload = await response.json();
  return data;
...
export const onResetPassword = createAsyncThunk<
  ...
  TUser,
  { rejectValue: TError }
>("user/onResetPassword", async function (user, { rejectWithValue }) {
  const response = await resetPasswordRequest(user);
  if (!response.ok) {
    return rejectWithValue({
      status: response.status,
      message: "Server Error, take a look on method onResetPassword",
    });
  }
  const data: TResponseWithoutPayload = await response.json();
  return data;
...
export const onPlaceOrder = createAsyncThunk<
  ...
  string[],
  { rejectValue: TError }
>("constructor/onPlaceOrder", async function (cart, { rejectWithValue }) {
  const response = await placeOrderRequest(cart);
  if (!response.ok) {
    return rejectWithValue({
      status: response.status,
      message: "Server Error, take a look on method onPlaceOrder",
    });
  }
