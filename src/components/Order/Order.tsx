    ...
    if (orderInfo && ingredients) {
        return (
            <li className={`pl-6 pr-6 pb-6 mr-2 ${styles.order}`}>
                ...
                <p className={`text text_type_main-medium ${styles.orderName}`}>{`${orderInfo?.name}`}</p>

                {
                    user && location.pathname === '/profile/orders' && (<p className={`text text_type_main-small ${styles.orderStatus} ${orderInfo?.status === 'done' ? 'text_color_success' : null}`}>{`${orderInfo?.status === 'done' ? 'Выполнен' : 'Готовится'}`}</p>)
                }
