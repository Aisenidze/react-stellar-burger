    ...
    <div className={` ${styles.ordersSummaryContainer}`}>
      <article className={`mb-15 ${styles.ordersBoard}`}>
        <div className={` ${styles.board}`}>
          <p className={`mb-6 text text_type_main-medium ${styles.boardTitle}`}>
            {" "}
            Готовы:
          </p>
            ...
            {readyOrders?.map(
              (order, index) =>
                index < 20 && (
                  <li
                    key={order}
                    className={`text text_type_digits-default text_color_success ${styles.boardItem}`}
                  >
                    {order}
                  </li>
                )
            ...
            {notReadyOrders?.map(
              (order, index) =>
                index < 20 && (
                  <li
                    key={order}
                    className={`text text_type_digits-default ${styles.boardItem}`}
                  >
                    {order}
                  </li>
                )
      ...
      <p className={`mb-15 text text_type_digits-large ${styles.ordersNumber}`}>
        {totalOrders}
      </p>
