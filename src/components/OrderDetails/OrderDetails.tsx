...
const OrderDetails: FC = () => {
  ...
  const returnIngredientsPrice = useCallback(() => {
    const arrOfIngredientsPrice = orderMatch?.ingredients?.map(
      (ingredient) =>
  ...
  if (orderMatch) {
    return (
      <div className={`pl-10 pr-10 pb-10 ${styles.order}`}>
        <p
          className={`mt-15 mb-10 text text_type_digits-default ${styles.orderNumber}`}
        >
          {`#${orderMatch.number}`}
        </p>
        <p
        ...
        <p
          className={`mt-2 mb-15 text text_type_main-small ${
            styles.orderStatus
          } ${orderMatch.status === "done" ? "text_color_success" : null}`}
        >
          {`${orderMatch.status === "done" ? "Выполнен" : "Готовится"}`}
        </p>
        <p className={`mb-6 text text_type_main-medium ${styles.orderName}`}>
          Состав:
        </p>
          ...
          {returnIngredients.map((ingredient, index) => (
            ...
            >
              <div className={`${styles.ingredientImageBlock}`}>
                ...
                />
                <p className={`text_type_main-small ${styles.ingredientName}`}>
                  {ingredient?.name}
                </p>
              </div>
