...
const BurgerConstructor: FC<IBurgerConstructorProps> = ({ onDropHandler }) => {
  ...
  const burgerConstructor = useAppSelector((store) => store.burgerConstructor) || {} || undefined;
  ...
  const total = useMemo(() => {
    if (burgerConstructor.bun && burgerConstructor.ingredients) {
      return (
        ...
        Array.from(burgerConstructor?.ingredients).reduce(
          (accum: number, item: TIngredient) => {
            if (item) {
              accum += item.price;
            }
            return accum;
  ...
  return (
    <form
      ...
      className={classNames(styles.form, `mt-25 ml-4`, {
    ...
    >
      {burgerConstructor?.bun && (
        <div
          className={classNames(styles.burgerConstructor__item, "mb-4 pr-2")}
        >
          <ConstructorElement
            type="top"
            isLocked={true}
      ...
      {burgerConstructor?.bun && (
        <div
          className={classNames(styles.burgerConstructor__item, "mt-4 pr-2")}
        >
          <ConstructorElement
            type="bottom"
            isLocked={true}
      ...
      <div
        className={classNames(
          styles.burgerConstructor__checkout,
          `mt-10 pb-10`
        )}
      >
        {burgerConstructor?.bun && burgerConstructor?.ingredients && (
          <div className={styles.burgerConstructor__total}>
            <p
              className={classNames(
                styles.burgerConstructor__ordersum,
                `mr-2 text text_type_main - large`
              )}
            >
              {total}
            </p>
      ...
      {!user && (
        <p
          className={classNames(
            styles.burgerConstructor__text,
            `text text_type_main-default text_color_inactive`
          )}
        >
          Для того чтобы оформить заказ нужно&nbsp;
          <Link
            className={classNames(
              styles.burgerConstructor__link,
      ...
      {!burgerConstructor.bun && (
        <p
          className={classNames(
            styles.burgerConstructor__text,
            `mt-5 text text_type_main-default text_color_inactive`
          )}
        >
          Для оформления заказа необходимо добавить булочку
        </p>
      )}
    </form>
  );
