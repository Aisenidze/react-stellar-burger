...
export const Login: FC = () => {
  ...
  const userErrorCode = userError?.message?.slice(
    userError?.message.length - 3,
  ...
  return (
    <div className={styles.login}>
      ...
      >
        ...
        {userError && (
          <p className={`mb-4 text text_type_main-default ${styles.textError}`}>
            {userErrorCode === "403"
              ? "Сессия истекла"
        ...
        {!user &&
          location.state?.from.pathname.startsWith("/profile/orders") && (
            <p
              className={`mb-4 text text_type_main-default ${styles.textError}`}
            >
              Войдите чтобы посмотреть заказы пользователя
            </p>
          )}
        ...
        <p
          className={`text text_color_inactive text_type_main-default ${styles.text}`}
        >
          Забыли пароль? &nbsp;
          <span>
            <Link
